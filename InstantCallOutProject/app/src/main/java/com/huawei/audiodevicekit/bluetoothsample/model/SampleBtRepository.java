...
public class SampleBtRepository implements SampleBtModel {
    ...
    public void sendCmd(String mac, int cmdType) {
        AudioBluetoothApi.getInstance().sendCmd(mac, cmdType, new IRspListener<Object>() {
            @Override
            public void onSuccess(Object object) {
                LogUtils.i(TAG, "onSuccess object = " + object);
                mCallback.onSendCmdResult(true, object);
            }

            @Override
            public void onFailed(int errorCode) {
                LogUtils.i(TAG, "onFailed errorCode = " + errorCode);
                mCallback.onSendCmdResult(false, errorCode);
            }
        });
    }
